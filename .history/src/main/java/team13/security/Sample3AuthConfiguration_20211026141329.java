...
public class Sample3AuthConfiguration extends WebSecurityConfigurerAdapter {
  ...
  protected void configure(AuthenticationManagerBuilder auth) throws Exception {
    ...
    auth.inMemoryAuthentication().withUser("customer1").password(passwordEncoder().encode("p@ss")).roles("CUSTOMER");

    // $ sshrun htpasswd -nbBC 10 admin adm1n
    // htpasswdでBCryptエンコードを行った後の文字列をパスワードとして指定している．
    auth.inMemoryAuthentication().withUser("seller1").password(passwordEncoder().encode("p@ss")).roles("SELLER");
